...
* 基本流程
    ...
    2.驗證學生、系辦助教的使用者身分，成功登入
    3.顯示服務選項
    4.選擇"加選"
    5.系統顯示要求使用者以課程代碼,課程名稱,教授,上課日期,上課節次搜尋欲加選的課程
    6.系統顯示搜尋到的課程代碼、課程名稱、星期、節次、教室、人數上限、開課人員
    7.使用者點擊加選按鈕
    8.系統將使用者的學號或身分為"系辦助教"的使用者選擇的學號、以及欲加選的課程傳到課程資料處存端
    9.後端回傳核准訊息
...
* 基本流程
    ...
    3. 系統顯示服務選項
    4. 使用者選擇退選選項
    5. 系統提示使用者以課程ID或課程名稱搜尋欲退選的課程
    6. 系統顯示搜尋到的課程名稱
    7. 使用者點選退選按鈕
    8. 系統顯示視窗確認是否退選
    9. 系統檢查總學分是否會低於9學分
    10. 系統將使用者欲退選課程的ID、名稱傳到後端資料庫進行核對
    11. 後端將退選成功訊息回傳到前端
* 替代流程
    2.1 於流程第2步，假設驗證使用者，帳號密碼錯誤
    
        1. 視窗顯示使用者重新輸入密碼
        2. 使用者重新輸入密碼
        3. 進行流程第2步
    8.1 於流程第8步，假設使用者取消退選
        
        1. 點選視窗"取消"按鈕
        2. 系統收到取消請求
        3. 回到流程第6步
    9.1 於流程第9步，若是總學分會低於9學分
        
        1. 視窗顯示"最低修習學分需為9學分"
        2. 系統拒絕退選請求
        3. 使用案例結束，退選失敗
    10.1 於流程第10步，若是課程為必修
